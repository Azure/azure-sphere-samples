...
def test__configure_network_proxy__null_address_throws_validation_error():
    ...
    with pytest.raises(exceptions.ValidationError, match="ERROR: Cannot configure proxy, address was null or empty"):
...
def test__configure_network_proxy__null_authentication_type_throws_validation_error():
    ...
    with pytest.raises(exceptions.ValidationError, match="ERROR: Cannot configure proxy, invalid authentication type"):
...
def test__configure_network_proxy__anonymous_with_username_and_password_throws_validation_error():
    ...
    with pytest.raises(exceptions.ValidationError, match="ERROR: Cannot configure proxy, incorrect configuration of authentication type and proxy credentials"):
        network.configure_proxy(False, "NotNullOrEmpty", 0, None,
