...
def test__add_wifi_network__null_ssid_throws_validation_error(fix_setup_networks):
    ...
    with pytest.raises(exceptions.ValidationError, match="ERROR: Cannot add wifi network config, invalid SSID"):
...
def test__add_wifi_network__null_config_state_throws_validation_error():
    ...
    with pytest.raises(exceptions.ValidationError, match="ERROR: Cannot add wifi network config, invalid config state"):
...
def test__add_wifi_network__null_security_state_throws_validation_error():
    ...
    with pytest.raises(exceptions.ValidationError, match="ERROR: Cannot add wifi network config, invalid network type"):
