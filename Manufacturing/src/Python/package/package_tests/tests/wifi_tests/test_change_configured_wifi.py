...
def test__change_wifi_network__null_config_state_returns_bad_request():
    ...
    assert error.exconly() == 'azuresphere_device_api.exceptions.ValidationError: ERROR: Cannot change Wi-Fi network config, config state is invalid.'
...
def test__change_wifi_network__null_psk_returns_bad_request():
    ...
    assert error.exconly() == 'azuresphere_device_api.exceptions.ValidationError: ERROR: Cannot change Wi-Fi network config, psk is null or empty.'
...
def test__change_wifi_network__change_non_existent_network_throws_device_error(fix_setup_networks):
    ...
    assert error.exconly() == 'azuresphere_device_api.exceptions.DeviceError: ERROR: An invalid request was made to the device. PSK is too short.'
