...
from azuresphere_device_api import certificate, image, sideload, wifi
...
def clean_images():
    ...
    if install_response != {}:
        raise requests.exceptions.ConnectionError(
            "Cannot clean images, installing images failed.")

    components = [app for app in get_image_components()
                  if app["image_type"] == 10]
