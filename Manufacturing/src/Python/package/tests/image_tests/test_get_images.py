...
from microsoft_azure_sphere_deviceapi import image, sideload
...
def test__get_images__add_image_reflected_in_get_images(fix_clean_images):
    ...
    assert not any(elem["uid"] == utils.blink_component_id for elem in response["components"])

    sideload.stage_image(utils.path_to_blink_image)
    sideload.install_images()

    end_response = image.get_images()
    assert any(elem["uid"] == utils.blink_component_id for elem in end_response["components"])
...
def test__get_images__add_then_delete_shows_removed_image(fix_clean_images):
    ...
    response = image.get_images()
    assert any(elem["uid"] == utils.blink_component_id for elem in response["components"])

    sideload.delete_image(utils.blink_component_id)

    end_response = image.get_images()

    assert not any(elem["uid"] == utils.blink_component_id for elem in end_response["components"])
