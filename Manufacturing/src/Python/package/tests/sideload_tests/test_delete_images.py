...
import pytest
from microsoft_azure_sphere_deviceapi import exceptions, sideload
...
def test__delete_image__null_component_throws_validation_error():
    ...
    assert error.exconly() == 'microsoft_azure_sphere_deviceapi.exceptions.ValidationError: ERROR: Cannot delete image, invalid component ID.'
...
def test__delete_image__delete_image_reflected_in_get_images(fix_clean_images):
    ...
    running_apps_start = len([item for item in images if item["image_type"] == 10])
    assert len([item for item in images if item["uid"] == utils.blink_component_id]) != 0

    sideload.delete_image(utils.blink_component_id)
    end_images = utils.get_image_components()
    assert len([item for item in end_images if item["uid"] == utils.blink_component_id]) == 0

    running_apps_end = len([item for item in end_images if item["image_type"] == 10])
