...
def test__add_wifi_network__null_ssid_throws_validation_error(fix_setup_networks):
    ...
    assert error.exconly() == 'microsoft_azure_sphere_deviceapi.exceptions.ValidationError: ERROR: Cannot add wifi network config, invalid SSID.'
...
def test__add_wifi_network__null_config_state_throws_validation_error():
    ...
    assert error.exconly() == 'microsoft_azure_sphere_deviceapi.exceptions.ValidationError: ERROR: Cannot add wifi network config, invalid config state.'
...
def test__add_wifi_network__invalid_config_state_throws_validation_error():
    ...
    with pytest.raises(exceptions.ValidationError) as error:
        wifi.add_wifi_network("NotNullOrEmpty", "", config_state="InvalidState!")
    assert error.exconly() == 'microsoft_azure_sphere_deviceapi.exceptions.ValidationError: ERROR: Cannot add wifi network config, invalid config state.'
...
def test__add_wifi_network__valid_config_state_returns_ok(fix_setup_networks):
    ...
    for count in range(len(valid_states)):
        wifi.add_wifi_network(f"TestSsid{count}", "open", "example", valid_states[count])
...
def test__add_wifi_network__null_security_state_throws_validation_error():
    ...
    assert error.exconly() == 'microsoft_azure_sphere_deviceapi.exceptions.ValidationError: ERROR: Cannot add wifi network config, invalid network type.'
...
def test__add_wifi_network__invalid_security_state_throws_validation_error():
    ...
    with pytest.raises(exceptions.ValidationError) as error:
        wifi.add_wifi_network("TestSsid", "InvalidSecurityState!", "example", "enabled")
    assert error.exconly() == 'microsoft_azure_sphere_deviceapi.exceptions.ValidationError: ERROR: Cannot add wifi network config, invalid network type.'
