...
def test__configure_network_proxy__null_address_throws_validation_error():
    ...
    assert error.exconly() == 'microsoft_azure_sphere_deviceapi.exceptions.ValidationError: ERROR: Cannot configure proxy, address was null or empty.'
...
def test__configure_network_proxy__null_authentication_type_throws_validation_error():
    ...
    assert error.exconly() == 'microsoft_azure_sphere_deviceapi.exceptions.ValidationError: ERROR: Cannot configure proxy, invalid authentication type.'
...
def test__configure_network_proxy__invalid_authentication_type_throws_validation_error():
    ...
    with pytest.raises(exceptions.ValidationError) as error:
        network.configure_proxy(False, "NotNullOrEmpty", 0, None, "NotNullOrEmpty")
    assert error.exconly() == 'microsoft_azure_sphere_deviceapi.exceptions.ValidationError: ERROR: Cannot configure proxy, invalid authentication type.'
...
def test__configure_network_proxy__anonymous_with_username_and_password_throws_validation_error():
    ...
    assert error.exconly() == 'microsoft_azure_sphere_deviceapi.exceptions.ValidationError: ERROR: Cannot configure proxy, incorrect configuration of authentication type and proxy credentials.'
...
def test__configure_network_proxy__anonymous_with_username_throws_validation_error():
    ...
    with pytest.raises(exceptions.ValidationError) as error:
        network.configure_proxy(False, "NotNullOrEmpty", 0, None, "anonymous", "username")
    assert error.exconly() == 'microsoft_azure_sphere_deviceapi.exceptions.ValidationError: ERROR: Cannot configure proxy, incorrect configuration of authentication type and proxy credentials.'
...
def test__configure_network_proxy__anonymous_with_valid_values_returns_correct_values(fix_clean_proxies):
    ...
    response = network.configure_proxy(True, "example.com", 8081, [], "anonymous")
...
def test__configure_network_proxy__basic_with_valid_values_returns_correct_values(fix_clean_proxies):
    ...
    response = network.configure_proxy(True, "example.com", 8081, [], "basic", "example", "example")
