...
1. Open the command-line interface using PowerShell, Windows command prompt, or Linux command shell.

1. Run `az sphere device enable-development`. This command sideloads the application debugging capabilities and GDB server, and moves the device to the Development device group. This command doesn't delete the application.
...
1. Delete the application from the device.
    ...
    1. Run `az sphere device image list-installed` to display the component ID of the application.
    1. Run the `az sphere device sideload delete` command to delete the application. For example:

        Azure Sphere extension for Azure CLI::

        ```
        az sphere device sideload delete --component-id <blink-component-id>
        ```
...
1. If you haven't already, run `az sphere product create --name MyProduct` at the command prompt. This command creates a product and the [standard device groups](https://learn.microsoft.com/azure-sphere/deployment/deployment-concepts#device-groups).

1. To assign the device to the **Field Test** device group. For example:

    Azure Sphere extension for Azure CLI:

    ```
    az sphere device assign --resource-group MyResourceGroup --catalog MyCatalog --target-product MyProduct --target-device-group "Field Test" --device <DeviceIdValue>
    ```
...
1. Run the `az sphere device wifi show-status` command to verify that the Azure Sphere device is connected to your WiFi network.
...
1. Upload the image package to your Azure Sphere catalog by using the following command:

    Azure Sphere extension for Azure CLI:
 
    ```
    az sphere image add --resource-group MyResourceGroup --catalog MyCatalog --image <path to your Blink folder>\Blink\out\ARM-Debug\Blink.imagepackage>
    ```
...
2. Create a new deployment for a device group for the uploaded images using the following command:

    Azure Sphere extension for Azure CLI:

    ```
    az sphere deployment create --resource-group MyResourceGroup --catalog MyCatalog --product MyProduct --device-group MyDeviceGroup --images <path to your Blink folder>\Blink\out\ARM-Debug\Blink.imagepackage>
    ```
...
1. Run `az sphere device image list-installed` to verify that the Blink/Hello World application is installed, and the Deferred Update application and GDB server are no longer installed.
