...
static void HandleReceivedMessage(event_data_t *eventData)
{
...
static void RequestTimeoutEventHandler(event_data_t *eventData)
{
...
static void SendUartMessage(event_data_t *eventData);
static event_data_t requestTimeoutEventData = {.eventHandler = &RequestTimeoutEventHandler};
static event_data_t uartReceivedEventData = {.eventHandler = &HandleReceivedMessage};
static event_data_t uartSendEventData = {.eventHandler = &SendUartMessage};
...
int MessageProtocol_Init(int epollFd)
{
    epollFdRef = epollFd;
    ...
    messageUartFd = UART_Open(MT3620_RDB_HEADER2_ISU0_UART, &uartConfig);
    if (messageUartFd < 0) {
