...
/// <summary>The message protocol message preamble. This should always be present at the start of a message.</summary>
...
///     All messages should begin with this header. It is not intended for use directly, but instead as
///     part of a complete message structure.
/// </summary>
typedef struct {
    /// <summary>Message preamble; must be set to <see cref="MessageProtocol_MessagePreamble" /></summary>
    uint8_t preamble[4];
...
typedef struct {
    ...
    ///     This must be the same as the sequence number in the request message this is a response to.
    /// </summary>
    MessageProtocol_SequenceNumber sequenceNumber;
